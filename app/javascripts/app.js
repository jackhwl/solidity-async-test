...
window.App = {
  ...
  basicInfoUpdate: async () => {
      let instance = await MyWallet.deployed();//.then(function(instance) {
  ...
  listenToEvent: async () => {
      ...
      instance.proposalReceived({}, {fromBlock:0, toBlock:'latest'}).watch(function(error, event){
          document.getElementById("propossalEvents").innerHTML += JSON.stringify(event);
      });
  },

  submitEtherToWallet: async () => {
    let instance = await MyWallet.deployed(); //).then(function(instance) {
    await instance.sendTransaction({from: account, to: instance.address, value: web3.toWei(5, "ether")});
  ...
  submitTransaction: async () => {
    ...
    try {
      let instance = await MyWallet.deployed();//.then(function(instance) {
      let result = await instance.spendMoneyOn(_to, web3.toWei(_amount, 'finney'), _reason, {from:accounts[1], gas:500000});
      //}).then(function(result) {
        console.log(result);
        App.basicInfoUpdate();
      //}).catch(function(err) {
