...
contract MyWallet is mortal {
    ...
    event proposalConfirmed(address indexed _from, address indexed _to, string _reason);
    ...
    function confirmProposal(uint proposal_id) public onlyowner returns (bool) {
        if(m_proposals[proposal_id]._from != address(0)) {
            if(m_proposals[proposal_id].sent != true) {
                ...
                proposalConfirmed(m_proposals[proposal_id]._from, m_proposals[proposal_id]._to, m_proposals[proposal_id]._reason);
                return true;
            }
        }
        return false;
